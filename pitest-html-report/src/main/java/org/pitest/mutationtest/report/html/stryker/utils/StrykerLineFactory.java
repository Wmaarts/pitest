...
public class StrykerLineFactory {

  private final Collection<MutationResult> mutations;

  public StrykerLineFactory(final Collection<MutationResult> mutations) {
    this.mutations = mutations;
  }

  public List<StrykerLine> convert(final Reader source) throws IOException {
    try {
      final StrykerInputStreamLineIterable lines = new StrykerInputStreamLineIterable(source);
  ...
  private Function<String, StrykerLine> stringToAnnotatedLine() {
    return new Function<String, StrykerLine>() {
      private int lineNumber = 1;

      @Override
      public StrykerLine apply(final String a) {
        final StrykerLine l = new StrykerLine(this.lineNumber, a,
